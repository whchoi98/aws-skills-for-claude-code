...
# Detect missing SKILL.md in .kiro/skills/ subdirectories
if [[ "$FILE_PATH" == .kiro/skills/* ]]; then
    DIR=$(dirname "$FILE_PATH")
    if [ ! -f "$DIR/SKILL.md" ] && [ "$DIR" != ".kiro/skills" ]; then
...
if [[ "$FILE_PATH" == .kiro/skills/* ]] || [[ "$FILE_PATH" == docs/architecture.md ]]; then
